 ...
 * @returns 按钮可见文本。
 */
export function cleanupButtonLabel(days: 7 | 30, count: number, state: CleanupButtonState): string {
 ...
 */
export async function reconcileDeletedSession(
  sessionId: string,
  deps: DeleteReconcileDeps,
 ...
 * @returns `null` 表示同步完成，否则返回刷新失败原因。
 */
export async function reconcileDeletedSessions(
  sessionIds: readonly string[],
  deps: DeleteManyReconcileDeps,
): Promise<unknown | null> {
