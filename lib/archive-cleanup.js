 ...
 * @returns 新映射及其是否发生变化。
 */
export function reconcileArchiveTimes(input) {
 ...
 */
export function eligibleArchivedSessionIds(archivedSessionIds, archivedAt, days, now) {
 ...
 * @returns 成功 id 与失败明细。
 */
export async function cleanupArchivedSessions(sessionIds, deleteOne) {
...
export class ArchiveTimeTracker {
    ...
    observe(archivedSessionIds) {
        ...
        this.tail = this.tail.then(async () => {
            ...
            if (result.changed)
                await this.options.store.write(result.archivedAt);
...
/** 版本 1 归档时间文件后端。 */
export class ArchiveTimeFileStore {
    path;
    ...
    async read() {
        ...
        const record = raw;
        if (record['version'] !== 1)
            throw new Error('archive time state must use version 1');
...
//# sourceMappingURL=archive-cleanup.js.map