window.__ModuleLoader__.load({
	id: "@dsh-external/dsh-archive-vault",
	factory: (require) => {
		var module = { exports: {} };
...
@media(max-width:680px){
  ...
  .av-actions{justify-content:flex-start}
...
`;
		...
		function buildPanel() {
			...
			function renderList() {
				...
				summary.textContent = rows.length === 0 ? "没有归档的会话" : `共 ${rows.length} 个归档会话${needle !== "" ? `，匹配 ${visible.length} 个` : ""} · 恢复后会话自动回到原位置`;
				if (rows.length === 0) {
		...
		function apply(ctx) {
			ctx.effect(() => ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				order: 61,
				label: () => "归档对话"
			}, ArchiveVaultPanel)), "@dsh-external/dsh-archive-vault: panel");
		}
		//#endregion
		exports.apply = apply;
