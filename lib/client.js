...
`;
		...
		function buildPanel(sessions) {
			...
			heading.append(el("h2", void 0, "归档清理"));
			...
			function renderSummary() {
				...
				summary.textContent = total === 0 ? "没有归档的会话" : `共 ${total} 个归档会话${unknownSuffix}`;
				renderCleanupButtons();
			}
			function bindCleanup(button, days) {
				button.addEventListener("click", () => {
					...
					fetchJson("/cleanup", {
						...
						const sessionRefreshError = await sessions.refresh().then(() => null, (error) => error);
						const summaryRefreshError = await loadSummary().then(() => null, (error) => error);
						if (failedCount > 0) {
							...
							say(`已删除 ${deletedCount} 个，但界面刷新失败；请刷新页面。\n${reason instanceof Error ? reason.message : String(reason)}`, "err");
						} else say(`已永久删除 ${deletedCount} 个归档超过 ${days} 天的对话。`, "ok");
					}).catch((error) => {
			...
			function loadSummary() {
				return fetchJson("/summary").then((data) => {
