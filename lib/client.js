window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* @returns 按钮可见文本。
		*/
		function cleanupButtonLabel(days, count, state) {
			if (state === "busy") return "清理中…";
			if (state === "armed") return `确认删除 ${count} 个`;
		...
		*/
		async function reconcileDeletedSession(sessionId, deps) {
		...
		* @returns `null` 表示同步完成，否则返回刷新失败原因。
		*/
		async function reconcileDeletedSessions(sessionIds, deps) {
...
`;
		...
		function archivedAgeText(archivedAt) {
			if (typeof archivedAt !== "number" || !Number.isFinite(archivedAt)) return null;
			return `已归档 ${Math.max(0, Math.floor((Date.now() - archivedAt) / (1440 * 60 * 1e3)))} 天`;
		}
		function buildPanel(sessions) {
			...
			heading.append(el("h2", void 0, "归档对话"));
			...
			function renderCleanupButtons() {
				for (const [days, button] of [[7, cleanup7Button], [30, cleanup30Button]]) {
					const count = cleanupCount(days);
					button.textContent = cleanupButtonLabel(days, count, busy ? "busy" : armedCleanup === days ? "armed" : "idle");
			...
			function renderList() {
				...
				const visible = needle === "" ? rows : rows.filter((row) => previewText(row).toLowerCase().includes(needle) || (row.workspaceTitle ?? "").toLowerCase().includes(needle) || (row.workspacePath ?? "").toLowerCase().includes(needle) || (row.cwd ?? "").toLowerCase().includes(needle) || row.sessionId.toLowerCase().includes(needle));
				const unknownSuffix = cleanup.unknownCount > 0 ? ` · ${cleanup.unknownCount} 个归档时间未知` : "";
				summary.textContent = rows.length === 0 ? "没有归档的会话" : `共 ${rows.length} 个归档会话${needle !== "" ? `，匹配 ${visible.length} 个` : ""}${unknownSuffix} · 恢复后会话自动回到原位置`;
				renderCleanupButtons();
				if (rows.length === 0) {
				...
				for (const row of visible) {
					...
					meta.append(el("span", void 0, formatTime(row.createdAt)));
					if (row.workspaceTitle !== null) {
						const tag = el("span", "av-tag", row.workspaceTitle);
						meta.append(tag);
					} else meta.append(el("span", "av-tag", "未分组"));
					...
					const idHint = el("span", void 0, `…${row.sessionId.slice(-8)}`);
					...
					deleteButton.addEventListener("click", () => {
						...
						if (!armed) {
							...
							disarmTimer = window.setTimeout(disarm, 5e3);
							return;
						}
						window.clearTimeout(disarmTimer);
						setBusy(true);
						...
						fetchJson("/delete", {
							...
							if (refreshError === null) {
								...
								if (listError === null) say(`已永久删除「${label}」及其会话日志。`, "ok");
								else say(`已永久删除「${label}」，但归档列表刷新失败；请刷新页面。\n${listError instanceof Error ? listError.message : String(listError)}`, "err");
							} else say(`已永久删除「${label}」，但左侧会话列表刷新失败；请刷新页面。\n${refreshError instanceof Error ? refreshError.message : String(refreshError)}`, "err");
						}).catch((error) => {
			...
			function bindCleanup(button, days) {
				button.addEventListener("click", () => {
					if (busy) return;
					if (cleanupCount(days) === 0) return;
					if (armedCleanup !== days) {
						disarmCleanup();
						armedCleanup = days;
						renderCleanupButtons();
						cleanupTimer = window.setTimeout(disarmCleanup, 5e3);
						return;
					}
					window.clearTimeout(cleanupTimer);
					armedCleanup = null;
					...
					fetchJson("/cleanup", {
						...
						const deletedIds = Array.isArray(data?.deletedSessionIds) ? data.deletedSessionIds.map(String) : [];
						...
						if (failedCount > 0) {
							const details = Array.isArray(data?.failures) ? data.failures.map((failure) => `${String(failure.sessionId)}：${String(failure.error)}`).join("\n") : "";
						...
						} else say(`已永久删除 ${deletedIds.length} 个归档超过 ${days} 天的对话。`, "ok");
					}).catch((error) => {
			...
			function loadArchiveData() {
				return fetchJson("/list").then((data) => {
		...
		function createArchiveVaultPanel(sessions) {
			return function ArchiveVaultPanel() {
		...
		function apply(ctx) {
			ctx.effect(() => ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				order: 61,
				label: () => "归档对话"
			}, createArchiveVaultPanel(ctx.sessions))), "dsh-archive-vault: panel");
		}
		//#endregion
		exports.apply = apply;
